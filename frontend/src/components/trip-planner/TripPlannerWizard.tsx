...
export function TripPlannerWizard() {
  ...
  const canAdvance =
    (currentStep === "destinations" && selections.destinations.length > 0) ||
    (currentStep === "duration" && selections.durationBucket !== null) ||
    (currentStep === "style" && selections.travelStyle !== null) ||
    (currentStep === "budget" && selections.budget !== null);
      ...
      {!isResultStep && (
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            ...
            <span className="font-medium text-primary">{STEP_LABELS[currentStep]}</span>
          </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {currentStep === "duration" && (
            <DurationSelector selected={selections.durationBucket} onSelect={setDuration} />
          )}
          {currentStep === "style" && (
            <TravelStyleSelector selected={selections.travelStyle} onSelect={setTravelStyle} />
          )}
          {currentStep === "budget" && (
            <BudgetSelector selected={selections.budget} onSelect={setBudget} />
          )}
